...
public class ProjectileStandard : MonoBehaviour
{
    ...
    [Header("Rewind")]
    [Tooltip("Check if gun is rewinder")]
    public bool isRewind = false;
    [Tooltip("Rewind rate for each projectile")]
    public float rewindPower;
    ...
    { 
        ...
        else
        {
            ...
            if (damageable)
            {
                Debug.Log("inflicting");
                damageable.InflictDamage(damage, false, m_ProjectileBase.owner);
            }

            if (isRewind)
            {
                Rewindable rewindable = collider.GetComponentInParent<Rewindable>();
                if (rewindable)
                {
                    rewindable.GetHit(rewindPower);
                }
            }
